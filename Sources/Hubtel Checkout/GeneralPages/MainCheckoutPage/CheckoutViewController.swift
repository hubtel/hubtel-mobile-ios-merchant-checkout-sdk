...
//        self.bottomButton.showLoader(value: true)
        ...
        let purchaseItem = PurchaseOrderItem(id: order?.clientReference, name: order?.businessName, quantity: 1, amount: order?.itemPrice, section: "checkout", provider: viewModel.providerChannel)
        let purchaseAnalytics = BeginPurchaseAppEvent(section: .checkout, amount: order?.itemPrice ?? 0.00, purchaseOrderItems: [purchaseItem])
